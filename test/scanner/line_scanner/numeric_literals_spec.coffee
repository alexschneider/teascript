  ...
  describe 'Extracting Numeric Literals', ->
    ...
    describe '#extractedNumericLiterals', ->
      ...
      context 'when a float is in scientific notation', ->
        lineScanner = new LineScanner '1.2E-123'
        extractionResult = lineScanner.extractedNumericLiterals()
      ...
      context 'when an integer is in scientific notation', ->
        lineScanner = new LineScanner '10E123'
        extractionResult = lineScanner.extractedNumericLiterals()

        it 'accurately increments the current position of the scanner', ->
          expect(lineScanner.position).to.equal 6

        it 'recognizes it as a integer', ->
          expect(lineScanner.lineTokens[0].kind).to.equal 'INTLIT'
